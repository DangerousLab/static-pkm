...
# - Vendor dependency management (MathJax self-hosting)
    ...
    cat > package.json << 'PACKAGEJSON'
{
  ...
  "scripts": {
    ...
    "setup:mathjax": "node scripts/download-mathjax.mjs"
  },
  "devDependencies": {
...
# Build system scripts (generated by setup-rollup.sh)
scripts/*
!scripts/generate-tree.mjs

# Build configurations (generated by setup-rollup.sh)
rollup.config.js
rollup.config.css.js
postcss.config.js

# Build documentation (generated by setup-rollup.sh)
ROLLUP-GUIDE.md
...
install_dependencies() {
    ...
    # Download vendor dependencies (MathJax)
    echo ""
    print_info "Downloading vendor dependencies (MathJax)..."
    if npm run setup:mathjax; then
        print_success "MathJax downloaded to vendor/mathjax/"
    else
        print_warning "Could not download MathJax (you can run 'npm run setup:mathjax' manually later)"
    fi
}
...
print_summary() {
    ...
    echo "  ✓ vendor/mathjax/ (self-hosted MathJax)"
    echo ""
    ...
    echo ""
    echo "  Update MathJax:"
    echo -e "     ${GREEN}npm run setup:mathjax${NC}"
    echo ""
